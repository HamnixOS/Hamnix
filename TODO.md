...
- [~] **KERNEL via LLVM — THE LAST ITEM** (USER: after all apps — now unblocked). Hard: bare-metal higher-half link, heavy **inline asm** passthrough, `%gs` **percpu** modeling, `clang -ffreestanding` + kernel.lds. Agent a-tbd scoping spike: compile a leaf kernel module via LLVM, map the gap classes (inline asm, %gs, barriers), produce a plan. NOT merging kernel codegen changes yet — scope first.
